...
export class ManualInputComponent implements OnInit {
  ...
  submit(val: string) {
    if (!val.includes('d') && !val.includes('b')) {
      this._messenger.publishMessage('SPLIT', parseInt(val), 'ManualInputComponent');
    }

    if (val.includes('d')) {
      this._messenger.publishMessage('DEATH', parseInt(val.split('d')[0]), 'ManualInputComponent');
    }

    if (val.includes('b')) {
      this._messenger.publishMessage('BONUS', parseInt(val.split('b')[0]), 'ManualInputComponent');
    }
